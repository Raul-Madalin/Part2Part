...
int sql_query() {
    ...
    sprintf(sql, "select available from test where name = '%s' and format = '%s';", global_file_name, global_file_format);
...
*/
    sprintf(screen_text, "%s", "Receiving request...");
    update_screen();

    if (read (tdL.cl, global_file_name, sizeof(global_file_name)) <= 0) {
        printf("[Thread %d]\n",tdL.idThread);
        perror ("Eroare la read() de la client.\n");
    }
    
    if (read (tdL.cl, global_file_format, sizeof(global_file_format)) <= 0) {
        printf("[Thread %d]\n",tdL.idThread);
        perror ("Eroare la read() de la client.\n");
    }
    
    sprintf(final_name, "%s.%s", global_file_name, global_file_format);
    ...
    global_file_size_int = atoi(server_file_size_string);
...
int client()
{
  ...
  if (new_request == 1) {
    ...
    printf("[CLIENT]: linked_request %d\n", new_request);
    new_port = first_port + network_node;
    if (new_port >= 5005) new_port -= 5;
    if (new_port == first_port) {
        printf("[CLIENT]: not found port%d\n", new_port);
        sprintf(screen_text, "%s\n%s",  "Server: Add or delete from database",
                                        "Client: Request file from starting node");
        update_screen();
        new_request = 1;
  ...
  while (connect (sd, (struct sockaddr *) &server,sizeof (struct sockaddr)) == -1 && new_port < 5010)
    ...
    sprintf(screen_text, "%s%d", "Connected to port: ", new_port);
    update_screen();
/*
    ...
    */
    ...
    sprintf(screen_text, "%s", "Sending request...");
    update_screen();

    sprintf(screen_text, "%s", "Waiting for response...");
    update_screen();
    ...
    sprintf(screen_text, "%s", client_response);
    update_screen();

    if (strcmp(client_response, "Found file") != 0) {
        printf("\n[FUCK] : %s\n", client_response);
        if (new_request == 1) first_port = new_port;
        new_request = 0;
        network_node++;
        close(sd);
        client(new_port);
    ...
    sprintf(received_name, "%s.%s", global_file_name, global_file_format);
    ...
    /* Receiving file size */
        ...
        received_file = fopen(received_name, "w");
...
void init_GUI(int argc, char **argv)
{
    ...
    nodes = gtk_combo_box_text_new();
        ...
        gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(nodes), "4");
        gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(nodes), "5");
}
